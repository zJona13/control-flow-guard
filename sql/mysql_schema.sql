-- =========================================================
--  ESQUEMA PARA MYSQL - CONTROL FLOW GUARD
-- =========================================================

-- Crear base de datos si no existe
CREATE DATABASE IF NOT EXISTS control_flow_guard CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE control_flow_guard;

-- 1) Tabla de usuarios (combina auth.users + perfiles de Supabase)
...
-- 2) Parámetros para MEA02: responsables por categoría + SLA (días)
...
-- 8) Trigger: asignación automática de responsable y fecha límite
...
FOR EACH ROW
BEGIN
  DECLARE v_responsable_id VARCHAR(36);
  DECLARE v_sla_dias INT;
  
  -- Obtener responsable y SLA para la categoría
  SELECT responsable_id, sla_dias INTO v_responsable_id, v_sla_dias
  FROM ti_responsables
  WHERE categoria = NEW.categoria
  LIMIT 1;
  
  -- Asignar responsable si no está definido
...
ALTER TABLE ti_responsables COMMENT = 'Tabla de ruteo: categoría -> responsable TI + SLA (días)';
ALTER TABLE control_excepciones COMMENT = 'Registro de excepciones de control (MEA02)';
ALTER TABLE excepcion_acciones COMMENT = 'Pasos/acciones correctivas y cambios de estado asociados a una excepción';
ALTER TABLE citas_contingencia COMMENT = 'Citas registradas en modo contingencia (DRP activo) para continuidad operacional';
